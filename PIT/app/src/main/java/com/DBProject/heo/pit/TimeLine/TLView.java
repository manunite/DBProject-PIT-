...
public class TLView extends View
{
    ...
    private void init()
    {
        ...
        blacktext15.setTextSize(40);

        blacktext30 = new Paint();          ///////////타임라인 날자, 요일 글자 크기
        blacktext30.setColor(Color.WHITE);
        blacktext30.setStrokeWidth(1f);
        blacktext30.setAntiAlias(true);
        blacktext30.setStyle(Style.FILL);
        blacktext30.setTextSize(50);
    ...
    protected void onDraw(Canvas canvas)
    {
        ...
        backgroundgradient = new Paint();           ////배경색 지
        backgroundgradient.setShader(new LinearGradient(0, 0, 0, getHeight(), Color.parseColor("#0a3b45"), 0x00000000, Shader.TileMode.CLAMP));
      ...
      /*// draw finger circles 손가락 터치하는곳 그리는거
      if (fingers > 0)
      {
        ...
        // draw ruler
        canvas.drawLine(0, rulery, width, rulery, blackline); //타임라인 중앙선
      ...
      */
        ...
        do
        {
            ...
            {
                // short month name
                canvas.drawText(monthnameshort, x+5, rulery-100, blacktext30);
            }
            else
            {
                // long month name
                canvas.drawText(monthnamelong, x+5, rulery-100, blacktext30);
            }
            ...
            for (int date = 1; date <= daysthismonth; date++, daynumber++, daymsx += DAY_MILLIS)
            {
                ...
                if (day_pixels < 10)
                {
                    // really tiny days
                    if (daynumber == 1)
                        canvas.drawLine(x, rulery-10, x, rulery+C, blackline);
                }
                else if (day_pixels < 60)
                {
                    // tiny days
                    if (daynumber == 1)
                        canvas.drawLine(x, rulery - 50, x, rulery + C, blackline);
                    else
                ...
                else if (day_pixels > 170)
                {
                    // big days
                    if (daynumber == 1)
                        canvas.drawLine(x, rulery - 100, x, rulery + C, blackline);
                    else
                        canvas.drawLine(x, rulery - 100, x, rulery + 10, blackline);

                    //drawstar(x, rulery - 100, 20, 20, canvas, blackline);
                    canvas.drawText(Integer.toString(date), x + 10, rulery - 60, blacktext30);
        ...
        if (calstuff != null)
        {
            ...
            for (int i = 0; i < calstuff.ourEvents.size(); i++)
            {
                ...
                //겹치는것이 위로 올라감
                gap = 0;
                int low = (int)max((float)0,(float)x);
                int up = (int)min((float) width, (float) x2);
                int maxindex = low;
                try {
                    for (int itr = low; itr <= up; itr++) {
                        if (arr[itr] > arr[maxindex])
                            maxindex = itr;
                    }
                    gap = arr[maxindex] * ((int) h + 10);
                }catch(Exception ex){
                }
                int y = rulery + 25 + gap;
                ...
                if ((e.title == null) || (day_pixels < 35))
                {
                    canvas.save();
                    canvas.clipRect(r);
                    canvas.drawText(e.title, x, y+h/2, blacktext20);
                    canvas.restore();
                }
                else if (day_pixels < 60)
                {
                    ...
                    //if (e.desc != null) if (e.desc.length()>0) canvas.drawText("...", x+h/2, y+3*h/2, blacktext20);
                }
                else
                {
                    String t = e.title;
        ...
        switch (motionevent.getActionMasked())
        {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                else if (fingers == 2)
                {
                    ...
                    span = max(right - left, DAY_MILLIS*3); // span of milliseconds in view
                    if(span > DAY_MILLIS*400)
                        span = DAY_MILLIS*400;
